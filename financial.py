...
def xnpv2(rate, cashflows, interval=30):
    """
    xnpv with an optional arg interval to specify cash flow intervals.
    """
    chron_order = sorted(cashflows, key = lambda x: x[0])
    t0 = chron_order[0][0] #t0 is the date of the first cash flow

    return sum([cf/(1+rate)**((t-t0).days/interval) for (t,cf) in chron_order])
...
def xirr2(cashflows, guess=0.1, interval=30):
    return optimize.newton(lambda r: xnpv2(r,cashflows, interval),guess)
